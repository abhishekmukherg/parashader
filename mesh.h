...
public:
  ...
  int numFaces() const { return quad_faces.size() + sphere_faces.size(); }
  Face* getFace(int i) {
    assert (i >= 0 && i < numFaces());
    if (i < numQuadFaces()) return quad_faces[i];
    else return sphere_faces[i-numQuadFaces()]; }
  vector<Face*>& getLights() { return lights; }
  const vector<Sphere*>& getSpheres() const { return spheres; }
  int numQuadFaces() const { return quad_faces.size(); }
...
private:
  ...
  void addSphere(const Vec3f &center, double radius, Material *material);
